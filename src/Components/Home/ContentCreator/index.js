...
import { Upload,Icon,Input,Button,message,Select,List,Spin,Tabs,Tag} from "antd";
...
export default props => (
 ...
 class ContentCreator extends Component {
  constructor(props) {
    super(props);
    this.state = {
      fileList: [],description:'',  loading:false,title:'',
      tags: ['Tag 1'],  inputVisible: false,  inputValue: '',
    };
  }
  handleUpload=()=>{
    this.setState({loading:true})
    let storeFileStorage=this.state.fileList.map((item,index)=>{
      let uploadFile=storage.ref('global-content').child(item.name).put(item)
      ...
      ()=> {
        ...
        let databasePush=db.ref('content').child('users').child('Notes').push({
    ...
    Promise.all(storeFileStorage).then(()=>{
      this.setState({loading:false,fileList:[],title:'',description:'',tags:[]})
    })
  }
  handleDelete=(index)=>{
  ...
  render() {
    ...
    return (
      <div className="home-main">
        ...
        <Input placeholder='title' style={{marginBottom:'5%'}}
          value={this.state.title} onChange={this.handleTitleChange}/>
        <TextArea rows={4}  style={{marginBottom:20}}
          placeholder='description ...' value={this.state.description}
          onChange={this.handleDescriptionChange}/>
        <div style={{ marginBottom: 16 }}>
          <TweenOneGroup enter={{scale: 0.8, opacity: 0, type: 'from', duration: 100,
        ...
        {inputVisible && (
          <Input ref={this.saveInputRef}  type="text"  size="small"
            style={{ width: 78,marginBottom:'5%' }}  value={inputValue}  onChange={this.handleTagInputChange}
            onBlur={this.handleTagInputConfirm}  onPressEnter={this.handleTagInputConfirm}  />
        )}
        {!inputVisible && (
